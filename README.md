...
flowchart TD
    ...
    H --> I["$create-pr<br/>Draft PR + PM links/comments"]
...
- `create-pr`: create draft PRs from `{pm-tool}/{task-ids}` branches, attach PM task URLs in PR bodies, comment linked PM tasks with PR URLs, report every PR URL, then run `review-pr`.
...
- `create-pr` creates draft PRs with PM task links in the PR body, comments linked PM tasks with PR URLs, then runs `review-pr`.
